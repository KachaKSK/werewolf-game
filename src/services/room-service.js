...
export async function startGame(roomId, currentHostId) {
    ...
    try {
        ...
        for (let i = 0; i < shuffledPlayers.length; i++) {
            if (availableRoles.length > 0) {
                ...
                if (playerIndex !== -1) {
                    ...
                    updatedPlayers[playerIndex]["chosen-image-url"] = getRoleImagePath(roleName, currentRoomData); // Assign role image
                ...
                if (playerIndex !== -1) {
                    ...
                    updatedPlayers[playerIndex]["chosen-image-url"] = getRoleImagePath("Villager", currentRoomData);
                }
        ...
        availableRoles.forEach(roleName => {
            centerRolePool.push({
                role: roleName,
                "chosen-image-url": getRoleImagePath(roleName, currentRoomData) // Assign image path for center roles
            });
        });
        ...
        updatedPlayers = updatedPlayers.map(player => {
            ...
            player["chosen-image-url"] = getRoleImagePath(player.role, currentRoomData);
            return player;
        });

        // Ensure center roles also have correct image paths
        centerRolePool.forEach(role => {
            role["chosen-image-url"] = getRoleImagePath(role.role, currentRoomData);
        });
